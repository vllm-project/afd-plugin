...
def test_padded_graph_capture_stops_before_the_memory_reserve():
    # At 8192 tokens the default ladder took 65 GiB per V4 FFN rank and the
    # NVSHMEM heap, set up after capture, had nothing left. Capture has to stop
    # before a layer that would cut into the reserve.
    from afd_plugin.v1.worker.cuda_graph import (
    ...
    gib = 2**30
    reserve = resolve_padded_ffn_graph_reserve_bytes({})
    assert reserve == 8 * gib
    assert resolve_padded_ffn_graph_reserve_bytes(
        {"AFD_FFN_GRAPH_MEM_RESERVE_GIB": "2.5"}
    ) == int(2.5 * gib)

    # The first layer is taken whenever the reserve is already there.
    assert padded_ffn_graph_layer_fits(
        free_bytes=9 * gib, per_layer_bytes=0, reserve_bytes=reserve
    )
    # A layer that would leave less than the reserve is not.
    assert not padded_ffn_graph_layer_fits(
        free_bytes=9 * gib, per_layer_bytes=2 * gib, reserve_bytes=reserve
    )
    # Exactly the reserve left over is fine.
    assert padded_ffn_graph_layer_fits(
        free_bytes=10 * gib, per_layer_bytes=2 * gib, reserve_bytes=reserve
    )

    with pytest.raises(ValueError):
        resolve_padded_ffn_graph_reserve_bytes({"AFD_FFN_GRAPH_MEM_RESERVE_GIB": "-1"})
