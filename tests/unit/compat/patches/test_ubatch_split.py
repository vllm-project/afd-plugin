...
def test_request_aligned_split_token_prefers_halfway_boundary() -> None:
    # Requests of 192, 96, 96 tokens: boundaries at 192 and 288; the even
    # split of 192 sits exactly on the first one.
    assert request_aligned_split_token(np.array([192, 96, 96])) == 192


def test_request_aligned_split_token_takes_nearest_boundary() -> None:
    # Aligned boundaries at 64 and 288 for a 320-token batch; the halfway
    # point 160 is nearer 64 (96 away) than 288 (128 away).
    assert request_aligned_split_token(np.array([64, 224, 32])) == 64
...
def test_request_aligned_split_token_refuses_when_none_aligned() -> None:
    # Uniform decode of six requests: boundaries 1..5, none aligned. Splitting
    # at an unaligned one crashed DeepSeek-V4's compressor during FULL decode
    # capture, so the batch runs whole instead.
    assert request_aligned_split_token(np.array([1] * 6)) is None
...
def test_request_aligned_split_lands_on_request_boundary() -> None:
    # Two prefills of 304 and 96 tokens: the even token split (200) would cut
    # the first request; the patch must split at 304 instead.
    ...
    assert slices[0] == UBatchSlice(slice(0, 1), slice(0, 304))
    assert slices[1] == UBatchSlice(slice(1, 2), slice(304, 400))
    assert sum(s.num_tokens for s in slices_padded) == 400


def test_small_uniform_decode_runs_whole() -> None:
    # Four single-token decodes have boundaries 1, 2, 3 -- none aligned -- so
    # the batch is not divided. This is the shape that crashed FULL decode
    # capture on DeepSeek-V4 when it was split anyway.
...
def test_large_uniform_decode_splits_on_an_aligned_boundary() -> None:
    # Forty-eight single-token decodes: 16 and 32 are the aligned boundaries,
    # equidistant from the halfway point, so either is a valid split.
    ...
    assert slices is not None
    assert slices[0].token_slice.stop % 16 == 0
