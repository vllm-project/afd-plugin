...
struct HcclCombinOpParam {
    uint64_t workSpace;          // Address used for client/server communication
    uint64_t workSpaceSize;      // Size of the client/server communication buffer
    uint32_t rankId;             // Rank id of the current device
    uint32_t rankDim;            // Total number of ranks
    uint64_t winSize;            // Unused by ccu
    uint64_t windowsIn[HCCL_MTE_MAX_RANK_NUM];  // Unused by ccu; MTE data region
    uint64_t windowsOut[HCCL_MTE_MAX_RANK_NUM]; // Unused by ccu; MTE state region
    // for ccu
    uint64_t xnAddr;
    uint64_t ckeAddr;
    uint64_t msAddr;
